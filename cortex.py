import websocket #'pip3 install websocket-client' for install
...
class Cortex(Dispatcher):
    def __init__(self, user, debug_mode=False):   #debug mode = false to reduce logs printed in the console
    ...
    def do_prepare_steps(self):
        headsets = self.query_headset()     #find the emotiv headset
        ...
        if len(headsets) > 0:
            ...
            if result == True:
                self.authorize()                                 #generation of cortex access token
